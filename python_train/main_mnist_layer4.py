...
class Net(nn.Module):
    def __init__(self):
        ...
        # 3nd layer
        self.fc3 = BinarizeLinear(second_layer_size, second_layer_size)
        self.bn3 = nn.BatchNorm1d(second_layer_size)
        self.relu3 = nn.ReLU()
...
def test():
    ...
    for i in range(second_layer_size):
        ...
        for j in range(second_layer_size):
            val = round(fc2_weights[i][j].item(), 6)
    ...
    print("bn2.running_mean: ", bn2_running_mean.shape)
    for i in range(second_layer_size):
    ...
    fc3_weights = model.state_dict()['fc3.weight']
    fc3_bias = model.state_dict()['fc3.bias']

    for i in range(second_layer_size):
        print("{", end='')
        for j in range(second_layer_size):
            val = round(fc3_weights[i][j].item(), 6)
            if (j == second_layer_size - 1):
                print(str(val) + "f", end='')
            else:
                print(str(val) + "f" + ", ", end='')
    ...
    print("fc3_bias: ", fc3_bias.shape)
    for i in range(second_layer_size):
        val = round(fc3_bias[i].item(), 6)
        print(str(val) + "f" + ", ", end='')
    ...
    print("bn2: ========================================================")
    print("========================================================")
    bn3_running_mean = model.state_dict()['bn3.running_mean']
    print("bn3.running_mean: ", bn3_running_mean.shape)
    for i in range(second_layer_size):
        val = round(bn3_running_mean[i].item(), 6)
        print(str(val) + "f" + ", ", end='')

    print("\n--------------------------------------------------------------------------------")

    bn3_running_var = model.state_dict()['bn3.running_var']
    print("bn2.running_var_modified: ", bn3_running_var.shape)
    for i in range(second_layer_size):
        val = round(math.sqrt(1 / (bn3_running_var[i].item() + 0.00001)), 6)
        print(str(val) + "f" + ", ", end='')
